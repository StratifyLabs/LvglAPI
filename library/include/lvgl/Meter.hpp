...
public:
  ...
  class Scale {
  public:
    Scale(lv_meter_scale_t *scale) : m_scale(scale) {}

    const lv_meter_scale_t *scale() const { return m_scale; }
  ...
  class Indicator {
  public:
    Indicator(lv_meter_indicator_t *indicator) : m_indicator(indicator) {}

    const lv_meter_indicator_t *indicator() const { return m_indicator; }
  ...
  Indicator add_needle_line(Scale scale, u16 width, Color color, s16 r_mod) {
    return Indicator(
      api()->meter_add_needle_line(object(), scale.scale(), width, color.get_color(), r_mod));
  }
  ...
  Indicator add_local_scale_lines(Scale scale, Color start, Color end, s16 width_mod) {
    return Indicator(
      api()->meter_add_scale_lines(object(), scale.scale(), start.get_color(), end.get_color(), true, width_mod));
  }

  Indicator add_scale_lines(Scale scale, Color start, Color end, s16 width_mod) {
    return Indicator(
      api()->meter_add_scale_lines(object(), scale.scale(), start.get_color(), end.get_color(), false, width_mod));
  }
